...
📦 npm
```bash
npm install playwright-captcha
```
<br>
<br>
...
**`captcha`:** A Playwright `Locator` representing the image challenge.
...
**`input`:** A Playwright `Locator` representing the text input where the challenge response should be entered.
